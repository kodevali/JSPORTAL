...
const Dashboard: React.FC<DashboardProps> = ({ user, accessToken, isAuthorizing, onSyncRequest }) => {
   ...
   * Uses a signature that combines content and normalized time to catch duplicates across sources.
   */
    ...
    const fetchData = async () => {
      ...
      if (accessToken) {
        ...
        try {
          ...
          // CALENDAR SYNC
          const calRes = await fetch(`https://www.googleapis.com/calendar/v3/calendars/primary/events?timeMin=${new Date().toISOString()}&maxResults=20&singleEvents=true&orderBy=startTime`, {
            headers: { Authorization: `Bearer ${accessToken}` }
          });
          const calData = await calRes.json();
          
          // STRICT DEDUPLICATION: Birthday spams are caught by matching summary + normalized start timestamp
          setCalendarEvents(deduplicate((calData.items || []), (c: any) => {
            const startTime = c.start.dateTime || c.start.date || '';
            const normalizedTime = startTime ? new Date(startTime).getTime().toString() : 'no-time';
            return `${c.summary || 'no-title'}-${normalizedTime}`;
          }));
        } catch (err) {
  ...
  const labelText = "text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-200 flex items-center space-x-2";
    ...
    <div className="space-y-4 animate-fadeIn max-w-[1280px] mx-auto">
      <div className="flex items-center justify-between pb-2">
        <div className="flex flex-col">
          ...
          <p className="text-slate-800 dark:text-slate-200 text-xs font-bold max-w-lg truncate mt-1">{greeting}</p>
        </div>
      ...
      {!accessToken && !isAuthorizing ? (
        <div className={`${cardStyle} h-[420px] items-center justify-center space-y-6 text-center bg-[#044A8D]/5 border-dashed border-[#044A8D]/30`}>
          ...
          <div className="max-w-xs px-4">
            ...
            <p className="text-slate-900 dark:text-slate-200 text-sm mt-2 font-bold">Link your JS Bank environment to synchronize your real-time schedule and communications.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-5 h-full">
          <div className={`${cardStyle} md:col-span-8 h-[380px]`}>
            <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-900 transition-colors">
              <div className={labelText}>
                <div className="w-2.5 h-2.5 rounded-full bg-[#EF7A25] shadow-[0_0_5px_#EF7A25]"></div>
                <span className="text-slate-900 dark:text-white font-black">Priority Communications</span>
              </div>
              <span className="text-[9px] font-black text-slate-500 dark:text-slate-400 tracking-tighter uppercase">GMAIL SECURE</span>
            </div>
                ...
                emails.map((email) => (
                  <div key={email.id} className="p-4 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-xl transition-all cursor-pointer group hover:shadow-md">
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-[9px] font-black text-slate-600 dark:text-slate-200">{email.date}</span>
                    </div>
                    ...
                    <p className="text-[10px] text-slate-700 dark:text-slate-300 line-clamp-1 leading-tight font-medium italic">{email.snippet}</p>
                  </div>
                ))
              ) : (
                <div className="h-full flex items-center justify-center text-[10px] font-black uppercase tracking-widest text-slate-500">Inbox Zero</div>
              )}
            </div>
          ...
          <div className={`${cardStyle} md:col-span-4 h-[380px] bg-white dark:bg-[#044A8D] border-l-4 border-l-[#044A8D] dark:border-l-transparent text-slate-900 dark:text-white shadow-xl transition-all duration-300`}>
            <div className="px-6 py-4 border-b border-slate-200 dark:border-white/30 flex items-center justify-between bg-slate-50 dark:bg-transparent">
              <div className="flex items-center space-x-2">
                <div className="w-2.5 h-2.5 rounded-full bg-[#044A8D] dark:bg-[#FAB51D] shadow-sm"></div>
                <h2 className="text-[10px] font-black uppercase tracking-widest text-[#044A8D] dark:text-white">JS Sessions</h2>
              </div>
              <span className="text-[8px] font-black text-slate-600 dark:text-white uppercase tracking-widest">Today</span>
            </div>
            
            <div className="flex-1 overflow-y-auto p-5 space-y-5 scrollbar-hide">
              {calendarEvents.length > 0 ? (
                calendarEvents.map((event) => (
                  <div key={event.id} className="flex items-start space-x-4 border-l-2 border-slate-300 dark:border-white/30 hover:border-[#044A8D] dark:hover:border-[#FAB51D] pl-4 transition-all group cursor-pointer">
                    <div className="flex flex-col">
                      ...
                      <p className="text-[9px] text-slate-700 dark:text-blue-100 uppercase font-black tracking-widest mt-1.5 flex items-center space-x-2">
                        <span className="bg-slate-100 dark:bg-white/10 px-2 py-0.5 rounded border border-slate-200 dark:border-transparent">{new Date(event.start.dateTime || event.start.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                      </p>
                    </div>
                  </div>
                ))
                ...
                <div className="h-full flex flex-col items-center justify-center space-y-2">
                   <svg className="w-8 h-8 text-slate-300 dark:text-white/40" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                   <span className="text-[9px] font-black uppercase tracking-widest text-slate-500 dark:text-white/60">Queue Clear</span>
                </div>
              )}
            </div>
            
            <div className="p-4 bg-slate-50 dark:bg-blue-900 border-t border-slate-200 dark:border-transparent">
              <button className="w-full py-2.5 bg-[#044A8D] dark:bg-white/10 hover:bg-blue-800 dark:hover:bg-white/20 text-white rounded-lg text-[9px] font-black uppercase tracking-widest transition-all shadow-sm border border-slate-200 dark:border-white/20">Launch JS Calendar</button>
            </div>
          </div>
          ...
          <div className={`${cardStyle} md:col-span-8 h-[240px]`}>
            <div className="px-6 py-3 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
               ...
               <div className="flex items-center space-x-2">
                  ...
                  <span className="text-[8px] font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest">Load Monitor</span>
               </div>
            </div>
                ...
                <AreaChart data={[{n: 'M', v: 40}, {n: 'T', v: 30}, {n: 'W', v: 60}, {n: 'T', v: 80}, {n: 'F', v: 50}]}>
                  <defs>
                    <linearGradient id="colorV" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#044A8D" stopOpacity={0.25}/>
          ...
          <div className={`${cardStyle} md:col-span-4 h-[240px]`}>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2 scrollbar-hide">
              {tasks.length > 0 ? (
                tasks.map((task) => (
                  <div key={task.id} className="p-3 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 hover:border-[#EF7A25] dark:hover:border-[#EF7A25] rounded-xl flex items-center justify-between group transition-all shadow-sm">
                ...
                <div className="h-full flex items-center justify-center text-[10px] font-black uppercase tracking-[0.2em] text-slate-600 dark:text-slate-400">Queue Clear</div>
              )}
            </div>
          </div>
        </div>
