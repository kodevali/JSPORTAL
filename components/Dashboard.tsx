...
const Dashboard: React.FC<DashboardProps> = ({ user, accessToken, isAuthorizing, onSyncRequest }) => {
  ...
  /**
   * Enhanced deduplication to prevent repeats even if IDs differ.
   * Checks for unique combinations of content and time.
   */
    ...
    const fetchData = async () => {
      ...
      if (accessToken) {
        ...
        try {
          ...
          setCalendarEvents(deduplicate((calData.items || []), (c: any) => `${c.summary}-${c.start.dateTime || c.start.date}`));
        } catch (err) {
    ...
    <div className="space-y-4 animate-fadeIn max-w-[1280px] mx-auto">
      <div className="flex items-center justify-between pb-2">
        <div className="flex flex-col">
          ...
          <p className="text-slate-600 dark:text-slate-300 text-xs font-semibold max-w-lg truncate mt-1">{greeting}</p>
        </div>
        ...
        <div className="flex items-center space-x-3 bg-white dark:bg-[#1E293B] p-2 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm transition-colors duration-300">
          ...
          <div className="flex items-center space-x-3 pr-4 pl-1">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[9px] font-bold text-slate-500 dark:text-slate-300 uppercase tracking-tight">{weather.desc}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {!accessToken && !isAuthorizing ? (
        <div className={`${cardStyle} h-[420px] items-center justify-center space-y-6 text-center bg-[#044A8D]/5 border-dashed border-[#044A8D]/20`}>
          ...
          <div className="max-w-xs px-4">
            ...
            <p className="text-slate-700 dark:text-slate-300 text-sm mt-2 font-medium">Link your JS Bank environment to synchronize your real-time schedule, backlog, and priority communications.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-5 h-full">
          <div className={`${cardStyle} md:col-span-8 h-[380px]`}>
            <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900 transition-colors">
              ...
              <span className="text-[9px] font-bold text-slate-400 dark:text-slate-500 tracking-tighter">GMAIL SECURE</span>
            </div>
                ...
                emails.map((email) => (
                  <div key={email.id} className="p-4 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 border border-slate-100 dark:border-slate-700 rounded-xl transition-all cursor-pointer group hover:shadow-sm">
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-[9px] font-bold text-slate-500 dark:text-slate-300">{email.date}</span>
                    </div>
                    ...
                    <p className="text-[10px] text-slate-600 dark:text-slate-400 line-clamp-1 leading-tight italic">{email.snippet}</p>
                  </div>
                ))
              ) : (
          ...
          <div className={`${cardStyle} md:col-span-4 h-[380px] bg-[#044A8D] text-white shadow-xl shadow-blue-100/20`}>
            <div className="px-6 py-4 border-b border-white/20 flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <h2 className="text-[10px] font-black uppercase tracking-widest text-white">Sessions</h2>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto p-5 space-y-5 scrollbar-hide">
              {calendarEvents.length > 0 ? (
                calendarEvents.map((event) => (
                  <div key={event.id} className="flex items-start space-x-4 border-l-2 border-white/20 hover:border-[#FAB51D] pl-4 transition-all group cursor-pointer">
                    <div className="flex flex-col">
                      <h4 className="font-bold text-xs line-clamp-2 leading-tight text-white group-hover:text-[#FAB51D] transition-colors">{event.summary}</h4>
                      <p className="text-[9px] text-blue-100 uppercase font-black tracking-widest mt-1.5 flex items-center space-x-2">
                ...
                <div className="h-full flex flex-col items-center justify-center space-y-2">
                   ...
                   <span className="text-[9px] font-black uppercase tracking-widest text-white/50">No Events Today</span>
                </div>
              )}
            </div>
            <div className="p-4 bg-blue-900">
              <button className="w-full py-2.5 bg-white/10 hover:bg-white/20 text-white rounded-lg text-[9px] font-black uppercase tracking-widest transition-all border border-white/20">Launch JS Calendar</button>
            </div>
          </div>
          ...
          <div className={`${cardStyle} md:col-span-8 h-[240px]`}>
            <div className="px-6 py-3 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
               ...
               <div className="flex items-center space-x-2">
                  ...
                  <span className="text-[8px] font-black text-slate-500 dark:text-slate-300 uppercase tracking-widest">Load Monitor</span>
               </div>
            </div>
              ...
              {tasks.length > 0 ? (
                tasks.map((task) => (
                  <div key={task.id} className="p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-[#EF7A25] dark:hover:border-[#EF7A25] rounded-xl flex items-center justify-between group transition-all shadow-sm">
                    ...
                    <input type="checkbox" className="w-4 h-4 rounded border-slate-400 dark:border-slate-500 text-[#EF7A25] focus:ring-[#EF7A25] cursor-pointer bg-white dark:bg-transparent" />
                  </div>
                ))
              ) : (
                <div className="h-full flex items-center justify-center text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-600">Queue Clear</div>
              )}
            </div>
          </div>
        </div>
